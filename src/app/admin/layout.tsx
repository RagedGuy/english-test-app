...
import { LayoutDashboard, BarChart3, Users, LogOut, Menu, X } from "lucide-react";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const SidebarContent = () => (
    <>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.name} href={item.href} onClick={() => setSidebarOpen(false)}>
              <div className={`flex items-center gap-3 px-4 py-3 uppercase tracking-widest text-xs font-semibold transition-all duration-200 ${
                isActive
      ...
      <div className="p-6 border-t border-[#2a1f18]">
        <button onClick={handleLogout} className="flex items-center gap-3 text-gray-400 hover:text-[#D65A5A] transition-colors uppercase tracking-widest text-xs font-semibold w-full px-4 py-2 hover:bg-[#D65A5A]/10">
  ...
  return (
    <div className="min-h-screen flex text-white">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-64 border-r border-[#2a1f18] bg-[#050505]/80 backdrop-blur-xl flex-col fixed h-full z-40">
        <SidebarContent />
      </aside>
      ...
      {/* Main Content */}
      <main className="flex-1 md:ml-64 mt-12 md:mt-0 overflow-auto">
        {children}
      </main>
    </div>
  );
